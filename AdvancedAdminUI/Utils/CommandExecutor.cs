    ...
    public static class CommandExecutor
    {
        ...
        public static void InvokeCommand(string command)
        {
            ...
            try
            {
                ...
                AdvancedAdminUIMod.Log.LogInfo($"[CommandExecutor] Executing command: '{command}'");
                
                // Set the text first, then invoke onEndEdit (simulates typing + pressing Enter)
                _inputField.text = command;
                _inputField.onEndEdit.Invoke(command);
                
                // Also try onSubmit if available (some Unity versions use this)
                try
                {
                    _inputField.onSubmit?.Invoke(command);
                }
                catch { }
            }
            catch (Exception ex)
