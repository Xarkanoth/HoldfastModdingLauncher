    ...
    public class HoldfastScriptMod : IHoldfastSharedMethods, IHoldfastSharedMethods3
    {
        ...
        public void OnIsClient(bool client, ulong steamId) 
        {
            ...
            if (client)
            {
                ...
                // CRITICAL: Force re-registration with the new server's mod loader
                // The old ClientModLoaderManager was destroyed when we left the previous server
                // We need to register with the NEW ClientModLoaderManager
                AdvancedAdminUIMod.Log.LogInfo("[HoldfastScriptMod] New server connection - forcing re-registration...");
                ForceReRegister();
            }
            else
            {
                ...
                // CRITICAL: Force re-registration on disconnect so we properly register with the next server
                // The ClientModLoaderManager will be destroyed, so our registration is invalid anyway
                AdvancedAdminUIMod.Log.LogInfo("[HoldfastScriptMod] Disconnected - clearing registration for next server");
                ForceReRegister();
            }
