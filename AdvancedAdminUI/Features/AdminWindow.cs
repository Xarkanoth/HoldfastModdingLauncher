...
namespace AdvancedAdminUI.Features
{
    ...
    public class AdminWindow
    {
        ...
        private int _currentMinimapSize = 900; // Default size - larger for better visibility
        ...
        private void CreateForm()
        {
            _form = new Form
            {
                ...
                Size = new Size(1200, 1100), // Larger window for bigger minimap
                StartPosition = FormStartPosition.CenterScreen,
                FormBorderStyle = FormBorderStyle.Sizable,
                MinimumSize = new Size(800, 800), // Larger minimum size
                TopMost = false,
                ShowInTaskbar = true,
                BackColor = _bgColor,
            ...
            // Handle resize to update minimap
        ...
        private void UpdateFeatureList()
        {
            try
            {
                ...
                _minimapPictureBox.MouseLeave += (s, e) => { 
                    ...
                    if (_hoverInfoLabel != null) _hoverInfoLabel.Text = "Hover over a player for info";
                };
                ...
                {
                    Text = "Hover over a player for info",
                    Font = new Font("Segoe UI", 9),
        ...
        private void MinimapPictureBox_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (hoveredPlayer != null)
            {
                if (_lastHoveredPlayerId != hoveredPlayer.PlayerId)
                {
                    ...
                    // Update hover info label with full player info
                    _hoverInfoLabel.Text = $"{displayName} | {hoveredPlayer.ClassName ?? "?"} | {hoveredPlayer.FactionName ?? "?"}";
            ...
            else
            {
                ...
                {
                    _lastHoveredPlayerId = -1;
                    _hoverInfoLabel.Text = "Hover over a player for info";
        ...
        private void UpdateMinimapSize()
        {
            try
            {
                ...
                // Scale minimap to ~80% of window width (leave room for legend), with min/max bounds
                int formWidth = _form.ClientSize.Width;
                if (formWidth <= 0) formWidth = 1100; // Default (larger)
                
                int targetSize = (int)(formWidth * 0.80f) - 40;
                _currentMinimapSize = Math.Max(600, Math.Min(1400, targetSize));
                
                // Update container and picturebox if they exist
                if (_minimapContainer != null && !_minimapContainer.IsDisposed)
                {
                    _minimapContainer.Width = _currentMinimapSize + 180; // Extra space for legend
                    _minimapContainer.Height = _currentMinimapSize + 100; // Taller for hover info + selection buttons
                    ...
                    foreach (System.Windows.Forms.Control ctrl in _minimapContainer.Controls)
                    {
                        ...
                        {
                            ctrl.Location = new Point(_currentMinimapSize + 20, 10);
                        }
                    }
                }
                ...
                {
                    _playerCountLabel.Location = new Point(10, _currentMinimapSize + 15);
                }
                
                if (_hoverInfoLabel != null && !_hoverInfoLabel.IsDisposed)
                {
                    _hoverInfoLabel.Location = new Point(120, _currentMinimapSize + 15);
                }
                ...
                if (_minimapContainer != null)
                {
                    foreach (System.Windows.Forms.Control ctrl in _minimapContainer.Controls)
                    {
                        ...
                        {
                            ctrl.Location = new Point(10, _currentMinimapSize + 45);
                        }
                        else if (ctrl.Tag?.ToString() == "clear_button")
                        {
                            ctrl.Location = new Point(150, _currentMinimapSize + 45);
                        }
                    }
                }
            }
        ...
        private void UpdateMinimap()
        {
            ...
            // Detect if a new map photo was captured (reset happened)
            // When HasMapCapture goes from false to true, a new photo was taken
            bool hasCapture = minimapFeature.HasMapCapture;
            if (!_lastKnownHasMapCapture && hasCapture)
            {
                // New capture detected - clear old cached image
                ...
                AdvancedAdminUIMod.Log.LogInfo("[AdminWindow] New map capture detected - will load new background");
            }
