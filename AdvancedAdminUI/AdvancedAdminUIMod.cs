...
namespace AdvancedAdminUI
{
    ...
    [BepInPlugin("com.xarkanoth.advancedadminui", "Advanced Admin UI", "1.0.44")]
    public class AdvancedAdminUIMod : BaseUnityPlugin
    {
        ...
        // Server browser filter moved to LauncherCoreMod
        ...
        void Awake()
        {
            ...
            try
            {
                // FIRST: Apply Harmony patches to capture admin events BEFORE mod loader registration
                // This catches OnPlayerConnected and OnRCLogin that fire during connection
                AdvancedAdminUI.Utils.AdminEventPatches.ApplyPatches();
        ...
        private void InitializeFeatures()
        {
            ...
            // Server browser filter has been moved to LauncherCoreMod (always runs for all launcher users)
            ...
            Log.LogInfo($"Loaded {_features.Count} features | F3=UI | F9=Rescan | F10=Debug | F11=Reload | F12=Toggle");
        }
        ...
        public void DoUpdate()
        {
            ...
            // When this flag is set, we need to register ASAP (every frame until success)
            if (AdvancedAdminUI.Utils.HoldfastScriptMod.NeedsReRegistration())
            {
                _scriptModRegistered = false;
                
                // Try to register immediately (every frame until successful)
                bool registered = AdvancedAdminUI.Utils.HoldfastScriptMod.Register();
                if (registered)
                {
                    _scriptModRegistered = true;
                    ColoredLogger.Log(ColoredLogger.BrightGreen, "[AdvancedAdminUI] ✓ Re-registered with game after server change!");
                    
                    // Check if we captured any admin events before registration
                    AdvancedAdminUI.Utils.AdminEventPatches.CheckCapturedAdminStatus();
                }
                // Don't log failure here - it spams while loading
            }
            
            // Check registration status periodically (every 5 seconds) for steady-state
            // This catches cases where we somehow got deregistered
            if (Time.time - _lastRegistrationAttempt > 5.0f)
            {
                ...
                if (!_scriptModRegistered)
                {
                    ...
                    if (_scriptModRegistered)
                    {
                        ColoredLogger.Log(ColoredLogger.BrightGreen, "[AdvancedAdminUI] ✓ Successfully registered with game");
            ...
            // F10 = Debug status (always works, doesn't require RC login)
            ...
            {
                bool isRCLoggedIn = HoldfastScriptMod.IsRCLoggedIn();
                bool hasMaster = HoldfastScriptMod.HasMasterLogin();
                
                Log.LogInfo($"[Advanced Admin UI] F3 pressed - RC Login: {isRCLoggedIn}, Master: {hasMaster}, Script Registered: {_scriptModRegistered}");
                
                if (!isRCLoggedIn && !_hasShownRCLoginMessage)
                {
                    _hasShownRCLoginMessage = true;
                    Log.LogInfo("[Advanced Admin UI] RC login required. Type 'rc login <password>' in console (press ~ to open).");
                }
            }
        ...
        /// <summary>
        /// Shows debug status of the mod - useful for diagnosing issues
        /// </summary>
        private void ShowDebugStatus()
        {
            ColoredLogger.Log(ColoredLogger.BrightCyan, "═══════════════════════════════════════════");
            ColoredLogger.Log(ColoredLogger.BrightCyan, "  ADVANCED ADMIN UI - DEBUG STATUS");
            ColoredLogger.Log(ColoredLogger.BrightCyan, "═══════════════════════════════════════════");
            
            // Mod state
            ColoredLogger.Log(ColoredLogger.White, $"  Mod Active: {_modActive}");
            ColoredLogger.Log(ColoredLogger.White, $"  Script Registered: {_scriptModRegistered}");
            ColoredLogger.Log(ColoredLogger.White, $"  Is Still Registered: {HoldfastScriptMod.IsStillRegistered()}");
            
            // Connection state
            ColoredLogger.Log(ColoredLogger.White, $"  Client Connected: {HoldfastScriptMod.IsClientConnected()}");
            ColoredLogger.Log(ColoredLogger.White, $"  Player Count: {PlayerTracker.GetPlayerCount()}");
            
            // Login state
            bool isRCLoggedIn = HoldfastScriptMod.IsRCLoggedIn();
            bool hasMaster = HoldfastScriptMod.HasMasterLogin();
            ColoredLogger.Log(isRCLoggedIn ? ColoredLogger.BrightGreen : ColoredLogger.Yellow, $"  RC Logged In: {isRCLoggedIn}");
            ColoredLogger.Log(hasMaster ? ColoredLogger.BrightMagenta : ColoredLogger.White, $"  Master Login: {hasMaster}");
            
            // Feature states
            ColoredLogger.Log(ColoredLogger.White, "  Features:");
            lock (_featuresLock)
            {
                foreach (var kvp in _features)
                {
                    string status = kvp.Value.IsEnabled ? "✓" : "✗";
                    ColoredLogger.Log(ColoredLogger.White, $"    [{status}] {kvp.Value.FeatureName}");
                }
            }
            ...
            if (_uiFeature != null)
            {
                ColoredLogger.Log(ColoredLogger.White, $"  UI Visible: {_uiFeature.IsUIVisible}");
            }
            
            ColoredLogger.Log(ColoredLogger.BrightCyan, "═══════════════════════════════════════════");
            ColoredLogger.Log(ColoredLogger.White, "  Press F3 after RC login to open panel");
            ColoredLogger.Log(ColoredLogger.White, "  RC login: Type 'rc login <password>' in ~ console");
            ColoredLogger.Log(ColoredLogger.BrightCyan, "═══════════════════════════════════════════");
        }
